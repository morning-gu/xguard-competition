...
try:
    ...
    logger.warning("peft 未安装，LoRA 微调不可用")
...
def merge_config(config: dict, overrides: list) -> dict:
    """
    合并命令行覆盖到配置
...
def load_model_and_tokenizer(config: dict):
    ...
    if model_cfg.get("use_flash_attention_2", False):
        try:
            ...
            logger.warning("Flash Attention 2 未安装，将使用默认注意力实现")
...
def prepare_data(config: dict, tokenizer: AutoTokenizer):
    ...
    准备训练和验证数据集
    """
    from src.data.dataset import XGuardTrainDataset, preprocess_raw_data
    ...
    if not os.path.exists(raw_data_path):
        ...
        logger.info("请先运行数据下载脚本或设置 auto_download=True")
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    train_from_config(args.config, args.overrides)
